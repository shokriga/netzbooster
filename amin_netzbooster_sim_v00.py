...
#     os.chdir("/home/ws/bw0928/mnt/lisa/netzbooster")
    ...
    snakemake.output = ["networks/sn50/postnetwork4.nc"]
...
def save_results_as_csv(n, snapshots):
    ...
    pd.concat([P_pos, P_neg],axis=1).to_csv("results/sn50/P_netzbooster_capacity.csv")
    ...
    p_pos.to_csv("results/sn50/p_pos.csv")
    p_neg = n.sols.Bus.pnl["p_neg"].loc[snapshots]
    p_neg.to_csv("results/sn50/p_neg.csv")

    # save constraints with multiindex as csv and remove from network
    keys_to_remove = []
    for key in n.buses_t.keys():
        n.buses_t[key].loc[snapshots].to_csv("results/sn50/{}.csv".format(key))
...
# for testing only consider 20 snapshots
